 ...
 * @author santi
 */
public class InnerRuinsLevelGenerator {
...
//        backgroundPatterns.add(new LevelChunk("rooms-inner-ruins/background1.tmx"));
//        backgroundPatterns.add(new LevelChunk("rooms-inner-ruins/background2.tmx"));
        roomPatterns[0].add(new LevelChunk("rooms-inner-ruins/room1-1.tmx"));
        roomPatterns[0].add(new LevelChunk("rooms-inner-ruins/room1-2.tmx"));
        roomPatterns[0].add(new LevelChunk("rooms-inner-ruins/room1-3.tmx"));
        roomPatterns[1].add(new LevelChunk("rooms-inner-ruins/room2-1.tmx"));
        roomPatterns[1].add(new LevelChunk("rooms-inner-ruins/room2-2.tmx"));
        roomPatterns[1].add(new LevelChunk("rooms-inner-ruins/room2-3.tmx"));
        roomPatterns[2].add(new LevelChunk("rooms-inner-ruins/room3-1.tmx"));
        roomPatterns[2].add(new LevelChunk("rooms-inner-ruins/room3-2.tmx"));
        roomPatterns[2].add(new LevelChunk("rooms-inner-ruins/room3-3.tmx"));
        roomPatterns[3].add(new LevelChunk("rooms-inner-ruins/room4-1.tmx"));
        roomPatterns[3].add(new LevelChunk("rooms-inner-ruins/room4-2.tmx"));
        roomPatterns[3].add(new LevelChunk("rooms-inner-ruins/room4-3.tmx"));
        roomPatterns[4].add(new LevelChunk("rooms-inner-ruins/room5-1.tmx"));
        roomPatterns[4].add(new LevelChunk("rooms-inner-ruins/room5-2.tmx"));
        roomPatterns[4].add(new LevelChunk("rooms-inner-ruins/room5-3.tmx"));
        roomPatterns[5].add(new LevelChunk("rooms-inner-ruins/room6-1.tmx"));
        roomPatterns[5].add(new LevelChunk("rooms-inner-ruins/room6-2.tmx"));
        roomPatterns[5].add(new LevelChunk("rooms-inner-ruins/room6-3.tmx"));

        bottomRowRoomPatterns[0].add(new LevelChunk("rooms-inner-ruins/room1-bottom-1.tmx"));
        bottomRowRoomPatterns[0].add(new LevelChunk("rooms-inner-ruins/room1-bottom-2.tmx"));
...
//        bottomRowRoomPatterns[3].add(new LevelChunk("rooms-jungle/room4-bottom-1.tmx"));
        ...
        doubleRoomPatterns.add(new LevelChunk("rooms-inner-ruins/doubleroom1.tmx"));
        doubleRoomPatterns.add(new LevelChunk("rooms-inner-ruins/doubleroom2.tmx"));
    ...
    //                System.out.println(x + "," + y);
            ...
            if (start_y != height-1 && end_y != height-1) break;
        }while(true);
...
//        int enemies_layer[][] = new int[width*ROOM_WIDTH][height*ROOM_HEIGHT];
        ...
        // 5.2) Complete the ropes/cables/beams to reach the top:
        int rope[] = {134,135,136};
        for(int x = 0;x<width*ROOM_WIDTH;x++) {
            int filling = 0;
            for(int y = height*ROOM_HEIGHT-1;y>=0;y--) {
                if (m.layer1[x][y]>=128) filling = 0;
                for(int i = 0;i<rope.length;i++) {
                    if (m.layer1[x][y] == rope[i]) filling = 1;    // rope
                }
                if (m.layer1[x][y] == 59+1) filling = 2;    // left beam
                if (m.layer1[x][y] < 128) {
                    if (filling == 1) m.layer1[x][y] = 134;
                    if (filling == 2) {
        ...
        for(int x = 0;x<width*ROOM_WIDTH;x++) {
            for(int y = 0;y<height*ROOM_HEIGHT;y++) {
                ...
                // platforms:
                if (m.layer1[x][y] == 137+1) {
                    if (x>0 && !m.isPlatform(m.layer1[x-1][y])) {
                        m.layer1[x][y] = 136+1;
                    } else if (x<width*ROOM_WIDTH-1 && !m.isPlatform(m.layer1[x+1][y])) {
                        m.layer1[x][y] = 139+1;
                    } else {
                        if (r.nextInt(3)==0) m.layer1[x][y] = 138+1;
                    }
                }
                // bricks:
        ...
        // 5.4) fill the bottom with water:
        for(int x = 0;x<width*ROOM_WIDTH;x++) {
            int y = (height-1)*ROOM_HEIGHT+1;
            if (m.isBackground(m.layer1[x][y])) {
                m.layer1[x][y] = (129+1);
            }
        }
        for(int y = (height-1)*ROOM_HEIGHT+2;y<height*ROOM_HEIGHT;y++) {
            for(int x = 0;x<width*ROOM_WIDTH;x++) {
                if (m.isBackground(m.layer1[x][y]) || m.layer1[x][y] == 129+1) {
                    m.layer1[x][y] = (128+1);
                }
            }
        }
            ...
            for(int y = 2;y<height*ROOM_HEIGHT-2;y++) {
                ...
                */
                ...
                if ((x%2)==0 &&
                    ...
                    if (r.nextInt(32)==0) {
                        // place an alien:
                        enemies.add(new Enemy(x,y-1,Enemy.ALIEN,3));
                ...
                if ((y%16)==3 &&
                    ...
                    if (r.nextInt(48)==0) {
                        // place a sentinel:
                        enemies.add(new Enemy(x,y,Enemy.SENTINEL,2));
                    }
                }                
...
//            System.out.println("enemy " + e.type + " at " + e.x + "," + e.y);
        ...
        m.saveTMX("InnerRuinsMap.tmx", new String[]{"tiles-inner-ruins.png","enemies.png"});

        // Save the level as a Z80 asm file for using in the MSX engine:
        {
            FileWriter fw = new FileWriter(new File("InnerRuinsMap.asm"));
